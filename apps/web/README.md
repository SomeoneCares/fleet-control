...
**Plan before apply** (approvals, apply, outcome) and **Drift** resolution (accept / revert / ignore once / exception).
The other navigation items open a page naming the slice they arrive in.
...
- `src/lib/view.ts` — pure view logic (instance status, capability rows, plan phase, drift rows), unit-tested
- `src/components/` — the shell (navigation from build document §3.1) and the small UI kit
- `src/screens/` — Instances, ConnectDrawer, DriftModal, Blueprints, ApplyPlan, NotYet

Not yet: sign-in (the API trusts a placeholder user, so one person cannot give both production approvals),
discovery for API-only instances, restoring a snapshot from the UI, and the remaining Slice 1 screens
(Fleet Designer, Agent Studio, Audit log, Access).
